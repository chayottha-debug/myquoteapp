...
try:
    ...
    HAS_IMG_LIB = True # แก้เป็น True เพื่อไม่ให้ฟ้อง error ในบาง environment
...
# ==========================================
# PDF ENGINE (ปรับปรุงระยะห่างหัวข้อลูกค้า)
# ==========================================
def create_pdf(d, items_df, summary, sigs, remark_text, show_vat_line, doc_title="ใบเสนอราคา (QUOTATION)"):
    ...
    for _, row in valid_items.iterrows():
        ...
        vals = [str(index + 1), str(row.get('รายการ')), f"{q:,.2f}", safe_str(row.get('หน่วย')), f"{p:,.2f}", f"{dis:,.2f}" if dis > 0 else "-", f"{total:,.2f}"]
    ...
    sum_x_label, sum_x_val, sum_y = 130, 170, current_y
    ...
    if show_vat_line: print_sum_row("ภาษีมูลค่าเพิ่ม 7%:", summary['vat'])

    grand_total_val = summary['grand_total']
    pdf.set_xy(15, sum_y)
    pdf.cell(115, 6, f"({bahttext(grand_total_val)})", 0, 0, 'C')
        ...
        if i == 0 and images[i]:
            try:
                with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as tmp:
                    ...
                    tmp_path = tmp.name
                pdf.image(tmp_path, x=x_positions[i] - 9, y=y_sig - 22, w=60, h=25)
                os.remove(tmp_path)
            except: pass
...
def convert_pdf_to_image(pdf_bytes, format_type):
    if not HAS_IMG_LIB: return None, "กรุณาติดตั้งไลบรารี PyMuPDF Pillow"
    try:
        ...
        new_im.save(img_byte_arr, format="JPEG" if format_type.upper() == "JPG" else "PNG")
